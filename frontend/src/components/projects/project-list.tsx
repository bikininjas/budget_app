        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map((project) => {
            const progress = project.budget
              ? (Number(project.total_spent || 0) / Number(project.budget)) * 100
              : 0;
            ...
            return (
              <div key={project.id} className="card dark:bg-slate-800 dark:border-slate-700 p-4 sm:p-6">
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span
                        className={`font-medium ${
                          isOverBudget ? 'text-danger-600 dark:text-danger-400' : 'text-slate-700 dark:text-slate-200'
                        }`}
                      >
                    ...
                    <p
                      className={`text-xs mt-1 ${
                        isOverBudget ? 'text-danger-500 dark:text-danger-400' : 'text-slate-400 dark:text-slate-500'
                      }`}
                    >
                ...
                <div className="flex items-center justify-between text-xs text-slate-400 dark:text-slate-500 pt-3 border-t border-slate-100 dark:border-slate-700">
                  ...
                  >
                    {project.is_active ? 'Actif' : 'Terminé'}
                  </span>
                </div>
              </div>
            );
